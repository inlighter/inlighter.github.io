"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var core_1 = require("@angular/core");
var router_1 = require("@angular/router");
var task_service_1 = require("./task.service");
require("rxjs/add/operator/map");
var FooterComponent = (function () {
    function FooterComponent(taskService, route) {
        this.taskService = taskService;
        this.route = route;
    }
    FooterComponent.prototype.ngOnInit = function () {
        var _this = this;
        this.route.params.map(function (params) { return params['status']; })
            .subscribe(function (status) { _this.currentStatus = status || ''; });
    };
    FooterComponent.prototype.getRemainingCount = function () {
        return this.taskService.getRemaining().length;
    };
    FooterComponent.prototype.getCompletedCount = function () {
        return this.taskService.getCompleted().length;
    };
    FooterComponent.prototype.clearCompleted = function () {
        this.taskService.removeCompleted();
    };
    FooterComponent.prototype.hasItems = function () {
        return Boolean(this.taskService.taskList.length);
    };
    return FooterComponent;
}());
FooterComponent = __decorate([
    core_1.Component({
        selector: 'my-footer',
        templateUrl: './footer.component.html'
    }),
    __metadata("design:paramtypes", [task_service_1.TaskService, router_1.ActivatedRoute])
], FooterComponent);
exports.FooterComponent = FooterComponent;
//# sourceMappingURL=footer.component.js.map